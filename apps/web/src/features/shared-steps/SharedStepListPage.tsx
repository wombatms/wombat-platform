...
import { Share2, PanelRight } from "lucide-react";
...
import { useSharedStepList, type SharedStep } from "./useSharedStepList";
import { sharedStepColumns } from "./columns";
import { cn } from "@/lib/utils";

const FACET_FIELDS = ["tag"];

function PreviewPane({ ss }: { ss: SharedStep | null }) {
  if (!ss) {
    return (
      <aside
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-3 p-4">
        <p className="font-mono text-[11px] font-semibold" style={{ color: "var(--feedback-info-fg)" }}>
          {ss.wombat_id}
        </p>
        ...
        {ss.updated_at && (
          <p className="text-[12px] tabular-nums" style={{ color: "var(--fg-muted)" }}>
            Updated {new Date(ss.updated_at).toLocaleDateString()}
          </p>
        )}
      </div>
    </aside>
...
function ListSkeleton({ density }: { density: Density }) {
  ...
  return (
    <div className="flex flex-col" aria-busy="true" aria-label="Loading shared steps">
...
export function SharedStepListPage() {
  const { projectSlug = "" } = useParams<{ projectSlug: string }>();
  const navigate = useNavigate();
  const [density, setDensity, toggleDensity] = useDensity();
  const [previewOpen, , togglePreview] = usePreview("shared-steps");
  const [facets, setFacets] = useState<FacetValue[]>([]);
  const [selectedId, setSelectedId] = useState<string | undefined>();
  const [focused, setFocused] = useState<SharedStep | null>(null);
  ...
  const { data, isLoading, isError, error, refetch } = useSharedStepList(projectSlug, { tag });
  const rows = data?.data ?? [];
  const total = data?.total ?? 0;

  const handleRowClick = useCallback((row: SharedStep) => {
    setSelectedId(row.wombat_id);
    setFocused(row);
    navigate(`/p/${projectSlug}/shared-steps/${row.wombat_id}`);
          ...
          <div className="flex-1 min-w-0 overflow-hidden">
            {rows.length === 0 ? (
              <EmptyState
                ...
                title="No shared steps match"
                description={facets.length > 0 ? "Try clearing some filters." : "This project has no shared steps yet."}
                action={
                  facets.length > 0 ? (
              ...
              <EntityTable
                data={rows}
                columns={sharedStepColumns}
                onRowClick={handleRowClick}
                selectedId={selectedId}
                density={density}
                getRowId={(row) => row.wombat_id}
                aria-label="Shared step list"
                className="h-full"
              />
            )}
          </div>
          {previewOpen && <PreviewPane ss={focused} />}
        </div>
      )}
    </div>
  );
