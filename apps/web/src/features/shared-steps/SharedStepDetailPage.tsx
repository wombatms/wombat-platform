...
export function SharedStepDetailPage() {
  ...
  useShortcut("e", () => {
    if (canPropose && !openProposal) navigate(`/p/${projectSlug}/shared_step/${wombatId}/edit`);
        ...
        <div className="flex items-center gap-2">
          {canPropose ? (
            <button
              ...
              onClick={() => navigate(`/p/${projectSlug}/shared_step/${wombatId}/edit`)}
              title={openProposal ? "A proposal is already open" : "Edit this shared step"}
              className={cn(
