...
def test_run_start_happy_path(runner: CliRunner):
    ...
    respx.post(f"{API_BASE}/api/projects/{PROJECT}/runs").mock(return_value=Response(201, json={"data": detail}))
...
def test_run_start_env_not_found(runner: CliRunner):
    ...
    respx.get(f"{API_BASE}/api/projects/{PROJECT}/environments").mock(return_value=Response(200, json={"data": []}))
...
def test_run_record_pass(runner: CliRunner):
    ...
    batch_response = {"results": [{"case_id": CASE_ID, "ok": True, "revision": 1, "error": None}]}
    respx.post(f"{API_BASE}/api/projects/{PROJECT}/runs/{RUN_ID}/results").mock(
...
def test_run_record_fail_with_options(runner: CliRunner):
    ...
    batch_response = {"results": [{"case_id": CASE_ID, "ok": True, "revision": 1, "error": None}]}
    route = respx.post(f"{API_BASE}/api/projects/{PROJECT}/runs/{RUN_ID}/results").mock(
        return_value=Response(200, json=batch_response)
    )
...
def test_run_record_with_if_match(runner: CliRunner):
    ...
    batch_response = {"results": [{"case_id": CASE_ID, "ok": True, "revision": 2, "error": None}]}
    route = respx.post(f"{API_BASE}/api/projects/{PROJECT}/runs/{RUN_ID}/results").mock(
        return_value=Response(200, json=batch_response)
    )
...
def test_run_close_completed(runner: CliRunner):
    ...
    route = respx.post(f"{API_BASE}/api/projects/{PROJECT}/runs/{RUN_ID}/close").mock(
        return_value=Response(200, json={"data": detail})
    )
...
def test_run_close_api_error_409(runner: CliRunner):
    ...
    respx.post(f"{API_BASE}/api/projects/{PROJECT}/runs/{RUN_ID}/close").mock(
        return_value=Response(
            409,
...
def test_run_start_record_close_lifecycle(runner: CliRunner):
    ...
    respx.post(f"{API_BASE}/api/projects/{PROJECT}/runs").mock(return_value=Response(201, json={"data": detail_open}))
    respx.post(f"{API_BASE}/api/projects/{PROJECT}/runs/{RUN_ID}/results").mock(
        return_value=Response(200, json=batch_response)
    )
    respx.post(f"{API_BASE}/api/projects/{PROJECT}/runs/{RUN_ID}/close").mock(
        return_value=Response(200, json={"data": detail_closed})
    )
