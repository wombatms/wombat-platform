...
async def test_owner_passes(db_session, sample_run, sample_user):
    ...
    await assert_run_actor_authorized(
        db_session, run=sample_run, user_id=sample_user.id, token_id=None,
        is_admin=False,
    )


@pytest.mark.asyncio
async def test_permission_without_ownership_or_admin_is_blocked(
    db_session, sample_run, another_user
):
    with pytest.raises(HTTPException) as exc:
        await assert_run_actor_authorized(
            db_session, run=sample_run, user_id=another_user.id,
...
async def test_assignee_passes(db_session, sample_run, another_user):
    ...
    await Repository(db_session).add_assignee(
        run_id=sample_run.id, user_id=another_user.id,
