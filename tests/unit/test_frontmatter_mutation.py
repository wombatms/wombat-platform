...
def test_appends_without_duplicating_approver():
    body = {
        "frontmatter": {
            "review": {"state": "pending", "approved_by": ["Alice <a@x>"]}
        },
        "markdown": "",
    }
    out = mutate_review_block(
        body, approver_display_name="Alice", approver_email="a@x", action="approve"
    )
...
def test_direct_publish_sets_flag():
    ...
    out = mutate_review_block(
        body, approver_display_name="B", approver_email="b@x", action="direct_publish"
    )
